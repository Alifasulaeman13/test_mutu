...
@section('page-title', 'Tambah Formula')
...
<div class="dashboard-section">
    ...
    <div class="section-body">
        <form action="{{ route('master-indikator.formula.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="form-group">
                ...
                <select name="numerator_type" class="form-select @error('numerator_type') is-invalid @enderror">
                    <option value="count" {{ old('numerator_type') == 'count' ? 'selected' : '' }}>
                        Hitung Jumlah (1, 2, 3, dst)
                    </option>
                    <option value="sum" {{ old('numerator_type') == 'sum' ? 'selected' : '' }}>
                        Total Nilai (bisa menggunakan desimal)
                    </option>
                    <option value="boolean" {{ old('numerator_type') == 'boolean' ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="denominator_type" class="form-select @error('denominator_type') is-invalid @enderror">
                    <option value="count" {{ old('denominator_type') == 'count' ? 'selected' : '' }}>
                        Hitung Jumlah (1, 2, 3, dst)
                    </option>
                    <option value="sum" {{ old('denominator_type') == 'sum' ? 'selected' : '' }}>
                        Total Nilai (bisa menggunakan desimal)
                    </option>
                    <option value="boolean" {{ old('denominator_type') == 'boolean' ? 'selected' : '' }}>
                        Ya/Tidak (untuk checklist)
                    </option>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="calculation_type" class="form-select @error('calculation_type') is-invalid @enderror">
                    ...
                    <option value="ratio" {{ old('calculation_type') == 'ratio' ? 'selected' : '' }}>
                        Rasio (pembilang dibagi penyebut)
                    </option>
                    <option value="average" {{ old('calculation_type') == 'average' ? 'selected' : '' }}>
                ...
                <x-formula-preview 
                    :initialType="old('calculation_type', 'percentage')"
                    :initialNumerator="old('numerator_label', '')"
                    :initialDenominator="old('denominator_label', '')"
                />
            </div>
            ...
            <div class="form-group calculation-multiplier">
                ...
                <input type="number" name="multiplier" class="form-input @error('multiplier') is-invalid @enderror" 
                       value="{{ old('multiplier', 100) }}" min="0" max="100" step="0.01">
                @error('multiplier')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
